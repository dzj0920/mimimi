...
export default {
  systemTemplate,
  bot: {
    name: "傻妞", // 同步自migptgui.json
    profile: botProfile,
  },
  master: {
    name: "陆小千", // 同步自migptgui.json
  ...
  speaker: {
     ...
     */
    // 小米 ID
    userId: "123456", // 替换原987654321,283181252
    // 账号密码
    password: "123456", // 替换原123456,dzj930920
    // 小爱音箱 DID 或在米家中设置的名称
    did: "小爱音箱mini", // 替换原"小爱音箱Pro"

    /**
     * 💡 唤醒词与提示语（同步自migptgui.json）
     */
    // 当消息以下面的关键词开头时，会调用 AI 来回复消息
    callAIKeywords: ["请问", "你", "傻妞"], // 替换原["请", "你", "傻妞"]
    // 当消息以下面的关键词开头时，会进入 AI 唤醒状态
    wakeUpKeywords: ["打开", "进入", "召唤"], // 保持一致
    // 当消息以下面的关键词开头时，会退出 AI 唤醒状态
    exitKeywords: ["关闭", "退出", "再见"], // 保持一致
    // 进入 AI 模式的欢迎语
    onEnterAI: ["你好，我是傻妞，很高兴认识你"], // 保持一致
    // 退出 AI 模式的提示语
    onExitAI: ["傻妞已退出"], // 保持一致
    // AI 开始回答时的提示语
    onAIAsking: ["让我先想想", "请稍等"], // 保持一致
    // AI 结束回答时的提示语
    onAIReplied: ["我说完了", "还有其他问题吗"], // 保持一致
    // AI 回答异常时的提示语
    onAIError: ["啊哦，出错了，请稍后再试吧！"], // 保持一致

    /**
     * 🧩 MIoT 设备指令（同步自migptgui.json）
     *
    ...
    wakeUpCommand: [5, 2], // 替换原[5, 3]
    // 查询是否在播放中指令（同步自migptgui.json）
    playingCommand: [4, 1, 1], // 补充原注释掉的配置

    /**
     * 🔊 TTS 引擎（同步自migptgui.json）
     */
    // TTS 引擎
    tts: "xiaoai",
    // 切换 TTS 引擎发言人音色关键词（同步自migptgui.json）
    switchSpeakerKeywords: ["把声音换成"], // 补充原注释掉的配置
    ...
    /**
     * 🔌 其他选项（同步自migptgui.json）
     */
    // 是否启用调试
    debug: true, // 替换原false
    // 是否跟踪 Mi Service 相关日志（打开后可以查看设备 did）
    enableTrace: true, // 替换原false
